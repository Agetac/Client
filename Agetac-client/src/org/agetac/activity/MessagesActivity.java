...
public class MessagesActivity extends AbstractActivity implements
	...
	public void onClick(View v) {
		...
		case R.id.buttonConsMess:
			...
			listAdapter = new SimpleAdapter(this.getBaseContext(),
					listMessages, R.layout.liste_message, new String[] {
							"titre", "description" }, new int[] { R.id.titre,
							R.id.description });

			listView.setAdapter(listAdapter);
			listMessages.clear();

			messagesRecus = controller.getInterventionEngine()
					.getListMessages();
			...
			for (int i = 0; i < messagesRecus.size(); i++) {
				...
				String d = TimeFormatter.getGroupeHoraire(messagesRecus.get(i)
						.getDate());

				map.put("titre", d);

				map.put("description", messagesRecus.get(i).getText()
						.toString());
			 ...
			 * listView.getItemAtPosition(position); AlertDialog.Builder adb =
